...
public class MailController implements IMailController {
	...
	public Response sendNewPassword(String oprId) {
		...
		try {
			...
			mailobj.sendMail(oprDTO.getOprEmail(), textHandler.mailMessage(oprDTO, newPass), "New Password");
			if (utils.DEV_ENABLED) utils.logMessage("Mail message: '" + textHandler.mailMessage(oprDTO, newPass) + "'");
