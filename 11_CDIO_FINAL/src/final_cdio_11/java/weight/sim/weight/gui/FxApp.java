package final_cdio_11.java.weight.sim.weight.gui;
...
public class FxApp extends Application {
		...
		try {
			...
			for (int i = 0; i < 10; i++) {
				...
				btnnum[i].setOnAction(new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent event) {
						onNumButtonPressed(inputHandler, btn);
					}
				});
			}
	...
	*/
	...
	public void setCursorPosition(int pos) {
		if (pos > userInput.getText().length()) throw new IndexOutOfBoundsException("Invalid position: " + pos + ". Text.length is: " + userInput.getText().length());
		if (pos < 0) throw new IndexOutOfBoundsException("Invalid position: " + pos + ". Must be 0 or higher.");
		caretPosition = pos;
	}
