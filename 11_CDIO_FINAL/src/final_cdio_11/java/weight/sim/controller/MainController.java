...
import final_cdio_11.java.weight.sim.socket.ISocketObserver;
import final_cdio_11.java.weight.sim.socket.SocketInMessage;
import final_cdio_11.java.weight.sim.socket.SocketOutMessage;
import final_cdio_11.java.weight.sim.weight.IWeightInterfaceController;
import final_cdio_11.java.weight.sim.weight.IWeightInterfaceController.InputType;
import final_cdio_11.java.weight.sim.weight.IWeightInterfaceObserver;
import final_cdio_11.java.weight.sim.weight.KeyPress;
...
public class MainController implements IMainController, ISocketObserver, IWeightInterfaceObserver {
	...
	private String handleUnit(String unit) {
		if (unit.startsWith("&1")) {
			weightController.changeInputType(InputType.UPPER);
			return unit.substring(2);
		}
		if (unit.startsWith("&2")) {
			weightController.changeInputType(InputType.LOWER);
			return unit.substring(2);
		}
		if (unit.startsWith("&3")) {
			weightController.changeInputType(InputType.NUMBERS);
			return unit.substring(2);
		}
		return unit;
